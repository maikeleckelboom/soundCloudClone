import {Blend, CustomColorGroup, hexFromArgb, Scheme, Theme, TonalPalette} from "@material/material-color-utilities";
...
function derivePaletteProperties(theme: Theme, {
    ...
    for (const [key, value] of Object.entries(theme.palettes)) {
        for (const tone of tones) {
            properties[`--${prefix ?? ''}${tokenize(key)}${tone}`] = hexFromArgb(value.tone(tone))
        }
    }
    return properties
}
...
function deriveCustomSchemeProperties(colorGroup: CustomColorGroup, options?: {
    ...
    for (const [key, value] of Object.entries(colorGroup[options?.dark ? 'dark' : 'light'])) {
        ...
        properties[`--${options?.prefix ?? ''}${token}${options?.suffix ?? ''}`] = hexFromArgb(value )
    }
    return properties
}
